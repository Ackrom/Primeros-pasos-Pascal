...
arreglo2=array[1..52] of string;
		procedure desglosadorL(var a:string;c:string;var b:array of string);
		var
		numero,i:integer;
begin
i:=0;
...
begin
	...
	reset(archivo);
	ciclo:=true;
	while not(eof(archivo)) and ciclo=true do
		begin
			read(archivo,buf);
				if buf.ci=ci then
					begin
...
assign(a,'C:\Users\Usuario\Desktop\TP3\TP1 INTEGRAL\ARCHIVOS\DPTO.DAT');
	...
	procedure clasificar(esc:arreglo1;var cap:arreglo1;buf_pv2:estudiantes;var buf_list:listado);
	var
	buf_pv2:estudiantes;
	buf_list:listado;
begin
